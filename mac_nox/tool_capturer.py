...
class main(object):
  def __init__(self):
    ...
    coord_muliplier = img.size[0]/screenres.width
    ...
    code += tab(2)+"xcode0,ycode0,xcode1,ycode1 = %d,%d,%d,%d"%(x0,y0,x0,y0) +"\n"
    code += tab(2)+"img = self.get_grab(image_grab)"    +"\n"
    
    code += tab(2)+"coord_multiplier = {}".format(coord_muliplier)+"\n"
    code += tab(2)+"# step = %d"%step+"\n"
    code += tab(2)+"coltol = %d"%10+"\n"
    
    #this is loop for in generated code
    code += tab(2)+"for y_code in range(ycode0-3,ycode1+3):" +"\n"
    code += tab(3)+"for x_code in range(xcode0-1,xcode1+1):" +"\n"
    code += tab(4)+"if ("+"\n"
    #this is loop for in captured image
    for y in range(y0,y1,step):
      for x in range(x0,x1,step):
        r,g,b,a = img.getpixel((x*coord_muliplier,y*coord_muliplier))
        
        if a!=255:
          raise Exception("alpha not 255")
        code += (
            tab(5)+
            ("self.is_color_similar(img.getpixel((x_code*coord_multiplier+cx(%d*coord_multiplier),y_code*coord_multiplier+cy(%d*coord_multiplier))),(%d,%d,%d),coltol) and" %
              (x-x0,y-y0, r,g,b)
            )+
            "\n"
          )
    ...
    code += tab(4)+"):"+"\n"
    code += tab(5)+"self.last_coord = x_code,y_code"+"\n"
